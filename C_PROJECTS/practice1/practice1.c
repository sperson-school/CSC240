#include<stdio.h>
#include<string.h>

/* This file will not compile until it is complete - you can comment out
 * any failing code that you haven't completed yet.
 */

int triple_it(int x);
int array_max(int a[], int len);
int letter_count(char a[], int len);
void printThem(int count);

/* Define practice1_t structure below */
struct practice1_t {
  char subject[4];
  int classnum;
  int hours;
};

int main(void) {
  int nums[11] = {1, 3, 5, 7, 9, 11, 10, 8, 6, 4, 2};
  char str[] = "CSC240 Fall 2019";
  struct practice1_t csc = {"CSC", 240, 5};
  struct practice1_t mat = {"MAT", 227, 4};

  printf("%s %d, %d hours", csc.subject, csc.classnum, csc.hours);
  printf("\n");
  printf("%s %d, %d hours", mat.subject, mat.classnum, mat.hours);
  printf("\n");



  printf("triple_it: ");
  for(int ii=0;ii<10;ii++) { 
    printf("%d ", triple_it(ii));
  }
  printf("\n");

  printf("\narray_max: %d\n", array_max(nums, 11));

  printf("\nletter_count: %d\n", letter_count(str, strlen(str)));

  printf("\nprintThem:\n");
  printThem(50);

  return 0;
}

int triple_it (int x) {
  return (x * 3);
}

int array_max(int a[], int len) {
  int largest = a[0];
  for (int i = 1; i < len; i++) {
    if (a[i] > largest) {
      largest = a[i];
    }
  }
  return largest;
}

int letter_count(char a[], int len) {
    int count = 0;

    for (int i = 0; i < len; i++) {
        if ((a[i] >= 'A' && a[i] <= 'Z') ||
            (a[i] >= 'a' && a[i] <= 'z')) {
            count++;
        }
    }

    return count;
}


void printThem(int count) {
    int printed = 0;

    for (int i = 1; i <= count; i++) {

        if (i % 7 == 0)
            continue;

        if (i % 15 == 0)
            printf("fifteen ");
        else if (i % 3 == 0)
            printf("three ");
        else if (i % 5 == 0)
            printf("five ");
        else
            printf("%d ", i);

        printed++;

        if (printed % 15 == 0)
            printf("\n");
    }

    printf("\n");
}

